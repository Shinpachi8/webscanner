...
def rand_str(length=6):
    return ''.join(random.sample('abcdefghjiklmnopqrstuvwxyz1234567890', length))
...
def verify(ip, port=80, name='', timeout=10, types='ip'):
    ...
    domain = url.replace(':', '_').replace('.','_') + rand_str()

    if not url.startswith('http:/') or not url.startswith('https:/'):
        posturl = 'http://' + url.rstrip('/')
    ...
    for payload in XXE_PAYLOAD:
        for path in ['/notify', '/weixin/notify']:
