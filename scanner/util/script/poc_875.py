...
def audit(arg):
    ...
    code, head, res, errcode, _ = hackhttp.http('-A "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.63 Safari/537.36" '+ url)
    if code == 200:
        m = re.search('<title>Sessions Example</title>', res)
        if m:
            info = {
                    "url": url,
                    "vuln_name": "Tomcat Sessions Example",
                    "severity": "low",
                    'proof': None
                    }
