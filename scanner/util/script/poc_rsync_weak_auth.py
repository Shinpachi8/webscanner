...
def verify(host, port=873, name='', timeout=5):
    ...
    PASSWORD_DIC = ['test', 'root', 'www', 'web', 'rsync', 'admin', '123456', '1234qwer', '1q2w3e4r']
    info = {
        "url": 'rsync://{}:{}'.format(host, port),
        "vuln_name": 'rsync weak password',
        "severity": 'high',
        "proof": ''
    }
    ...
    if not_unauth_list:
        info['proof'] = 'unauthorized access: {}'.format(not_unauth_list)
        #info += u'未授权访问目录有:%s;' %','.join(not_unauth_list)
    if weak_auth_list:
        for weak_auth in weak_auth_list:
            info['proof'] += u'\n目录%s存在弱验证:%s:%s;' %weak_auth
    if info['proof']:
        return info
...
if __name__ == '__main__':
    ...
    for ip_addr in ip_list:
        print(ip_addr, verify(ip_addr, 873))
